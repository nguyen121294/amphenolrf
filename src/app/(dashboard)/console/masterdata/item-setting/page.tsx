...
export default async function ItemSettingPage() {
  ...
  const hasViewAccess = await checkUserPermission(session.userId, role, "/console/masterdata/item-setting", "view");
  if (!hasViewAccess) {
    redirect("/console");
  }

  // Determine if the user is allowed to edit items (or create/delete, we pass edit as represents write permission)
  const canEdit = await checkUserPermission(session.userId, role, "/console/masterdata/item-setting", "edit");
