...
export async function getItemsAction(
  ...
  try {
    ...
    const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/item-setting", "view");
    if (!hasAccess) {
...
export async function saveItemAction(data: {
  ...
  const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/item-setting", actionType);
  if (!hasAccess) {
  ...
  try {
    if (id) {
      ...
      revalidatePath("/console/masterdata/item-setting");
...
export async function deleteItemAction(id: number): Promise<ActionResponse<void>> {
  ...
  const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/item-setting", "delete");
  if (!hasAccess) {
...
export async function importExcelAction(formData: FormData): Promise<ActionResponse<{ imported: number }>> {
  ...
  const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/item-setting", "import");
  if (!hasAccess) {
...
export async function exportExcelAction(): Promise<ActionResponse<string>> {
  try {
    ...
    const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/item-setting", "export");
    if (!hasAccess) {
