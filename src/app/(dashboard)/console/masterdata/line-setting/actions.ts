...
export async function getLinesAction(
  ...
  try {
    ...
    const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/line-setting", "view");
    if (!hasAccess) {
...
export async function saveLineAction(data: {
  ...
  const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/line-setting", actionType);
  if (!hasAccess) {
  ...
  try {
    if (id) {
      ...
      revalidatePath("/console/masterdata/line-setting");
...
export async function deleteLineAction(id: number): Promise<ActionResponse<void>> {
  ...
  const hasAccess = await checkUserPermission(session.userId, session.role, "/console/masterdata/line-setting", "delete");
  if (!hasAccess) {
