...
const CONFIGURABLE_PAGES = [
  ...
  { path: "/app/masterdata/item-setting", name: "Thiết lập Item (Master Data)" },
  { path: "/app/masterdata/line-setting", name: "Thiết lập Line (Master Data)" },
];
...
function getStaticDefaultPermission(role: string, pagePath: string) {
  ...
  if (isUser) {
    if (pagePath.startsWith("/app/masterdata")) {
      return {
        canView: true,
        canCreate: false,
        canEdit: false,
    ...
    if (pagePath.startsWith("/app/production")) {
      return {
        canView: true,
...
export async function saveUserPermissionsAction(
  ...
  try {
    ...
    revalidatePath("/app/users/permissions");
